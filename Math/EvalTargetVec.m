...
% Gimbal
time_interval = (1:200);
[encoder.time_us, ia, ic] = unique(dataTable.TARGET_INFO.TimeEncoder(time_interval),'stable');
encoder.phi_deg = dataTable.TARGET_INFO.Phi_Enc(ia);
encoder.theta_deg = dataTable.TARGET_INFO.Theta_Enc(ia);
encoder.psi_deg = dataTable.TARGET_INFO.Psi_Enc(ia);


Phi_Enc_spline = calc_SmoothingCubicSpline(encoder.time_us, encoder.phi_deg,0.999,0.999);
Theta_Enc_spline = calc_SmoothingCubicSpline(encoder.time_us, encoder.theta_deg,0.1,0.1);
Psi_Enc_spline = calc_SmoothingCubicSpline(encoder.time_us, encoder.psi_deg,1,1);

intrpl_p = ppval(Theta_Enc_spline,EncoderTime);
plot(EncoderTime,intrpl_p)
