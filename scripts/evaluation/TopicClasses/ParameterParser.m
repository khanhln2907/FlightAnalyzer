classdef ParameterParser
    %PARAMETERPARSER Summary of this class goes here
    %   Detailed explanation goes here
    
    properties (Constant)
        TEMP_FOLDER = fullfile(pwd, sprintf("AutoGeneratedFiguresFolder"));
    end
    ...
    methods (Static)
        ...
        function [path] = get_save_path_param(varargin)
           p = inputParser;
           addParameter(p, 'SavePath', ParameterParser.TEMP_FOLDER , @(x) exist(x, 'dir') ~= 0); % if failed, the path does not exist
           parse(p, varargin{:}); 
