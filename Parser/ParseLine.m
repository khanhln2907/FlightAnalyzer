function [isValid ,topicNameStr, payload] = ParseLine(str)    
    try
        %% Compute the checksum of the payload
        %% Decoded JSON
        decodedData = JSONDecode(str);
        topicName = fieldnames(decodedData);
        if(numel(topicName) > 2)
           %fprintf('Two topics in one line detected. Not yet handled: \n %s', str);
           assert(false);
        end
        
        CSStartIndex = strfind(str,',"CS"');
        payload = str(1:CSStartIndex-1);
        [CS1, CS2] = CalculateChecksum(payload);
       
        % Return the topic name and the payload
        isValid = (CS1 == decodedData.CS(1)) && (CS2 == decodedData.CS(2));
        topicNameStr = topicName{1};
        payload = decodedData.(topicNameStr);
    catch ME
        topicNameStr = "NONE";
        isValid = false;
        payload = [];
        %fprintf('JSONdecode Failed. Line: %s. Mes: %s\n', str, ME.message);
    end
end

